...
CREATE INDEX IF NOT EXISTS `idx_openai_object_refs_user_type_created_at` ON `openai_object_refs` (`user_id`, `object_type`, `created_at`);
CREATE INDEX IF NOT EXISTS `idx_openai_object_refs_user_id` ON `openai_object_refs` (`user_id`);
...
CREATE TABLE IF NOT EXISTS `upstream_channels` (
  ...
  `status_code_mapping` TEXT NULL,
  `model_suffix_preserve` TEXT NULL,
  `request_body_blacklist` TEXT NULL,
  `request_body_whitelist` TEXT NULL,
