...
// FeatureDisabledEffective 返回某个 feature_disable_* 的最终禁用状态（包含 app_settings_defaults 与 personal 模式硬禁用）。
//
// 约定：
// - 运行期配置（app_settings）优先于配置文件默认（app_settings_defaults）
// - personal 模式会强制禁用计费与工单
// - 当读取数据库失败时，回退到配置文件默认值（不因 DB 故障放大影响）
func (s *Store) FeatureDisabledEffective(ctx context.Context, personalMode bool, key string) bool {
	if s == nil {
		return false
	}
	switch key {
