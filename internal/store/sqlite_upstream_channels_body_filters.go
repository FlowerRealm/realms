...
func ensureSQLiteUpstreamChannelBodyFilterColumns(db *sql.DB) error {
	if db == nil {
	...
	need := []addCol{
		{name: "model_suffix_preserve", ddl: `ALTER TABLE upstream_channels ADD COLUMN model_suffix_preserve TEXT NULL`},
		{name: "request_body_blacklist", ddl: `ALTER TABLE upstream_channels ADD COLUMN request_body_blacklist TEXT NULL`},
		{name: "request_body_whitelist", ddl: `ALTER TABLE upstream_channels ADD COLUMN request_body_whitelist TEXT NULL`},
	}
	for _, c := range need {
