...
func (s *Server) UpdateChannelRequestBodyWhitelist(w http.ResponseWriter, r *http.Request) {
	...
	raw := strings.TrimSpace(r.FormValue("request_body_whitelist"))

	if err := s.st.UpdateUpstreamChannelRequestBodyWhitelist(r.Context(), channelID, raw); err != nil {
	...
	http.Redirect(w, r, returnTo+"?msg="+url.QueryEscape("请求体白名单已保存"), http.StatusFound)
}
