...
func (s *Server) UpdateChannelRequestBodyBlacklist(w http.ResponseWriter, r *http.Request) {
	...
	raw := strings.TrimSpace(r.FormValue("request_body_blacklist"))

	if err := s.st.UpdateUpstreamChannelRequestBodyBlacklist(r.Context(), channelID, raw); err != nil {
	...
	http.Redirect(w, r, returnTo+"?msg="+url.QueryEscape("请求体黑名单已保存"), http.StatusFound)
}
