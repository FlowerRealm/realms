...
func (s *Server) APIEmailVerificationSend(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = s.store.DeleteExpiredEmailVerifications(r.Context())
	if _, err := s.store.UpsertEmailVerification(r.Context(), nil, email, crypto.TokenHash(code), expiresAt); err != nil {
	...
	smtpCfg := s.smtpConfigEffective(r.Context())
