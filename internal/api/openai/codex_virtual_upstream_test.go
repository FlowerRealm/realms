...
func TestCodexVirtualUpstream_NonStream_PatchesQuotaAndForwardsXCodexHeaders(t *testing.T) {
	...
	h := NewHandler(fs, fs, sched, exec, nil, nil, false, nil, fakeAudit{}, nil, nil, upstream.SSEPumpOptions{})
...
func TestCodexVirtualUpstream_Stream_PatchesQuota(t *testing.T) {
	...
	sched := scheduler.New(fs)
	h := NewHandler(fs, fs, sched, exec, nil, nil, false, nil, fakeAudit{}, nil, nil, upstream.SSEPumpOptions{MaxLineBytes: 256 << 10, InitialLineBytes: 64 << 10})
