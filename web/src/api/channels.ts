...
export type ChannelModelProbeResult = {
  ...
  model_check_status?: "ok" | "mismatch" | "unknown";
};
...
export type ChannelProbeSummary = {
  ...
  sample?: string;
  latency_ms?: number;
  model_check_ok: number;
  model_check_mismatch: number;
  model_check_unknown: number;
...
export async function testChannel(
  ...
  const res = await api.get<APIResponse<{ latency_ms: number; probe?: ChannelProbeSummary }>>(
