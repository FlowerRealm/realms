...
> 你可以在「系统设置」开启 `feature_disable_models=true` 进入 model passthrough（会关闭 `GET /v1/models`；部分客户端可能依赖该接口）。自用模式（`self_mode.enable=true`）下数据面也会进入“宽松的 model passthrough”：不要求模型定义/定价；默认透传 `model`，但若配置了“渠道绑定模型”（alias/upstream_model），则仍会按选中渠道对 `model` 做转发改写。
...
- 自用模式：`self_mode.enable=true`  
  用于个人/小团队自用，禁用订阅/支付/工单等功能域，并让数据面进入 free mode（不校验订阅/余额，仅记录用量）；数据面对 `model` 采用宽松模式（不要求模型定义/定价，默认透传，绑定存在时仍可改写）。
