...
- Codex OAuth 上游（默认模式）：创建 Channel → 手动完成 OAuth 授权 → 复制浏览器回调 URL（含 `code/state`）并粘贴完成导入账号（自用模式下不提供 OAuth/账号链路）
...
> 你可以在「系统设置」开启 `feature_disable_models=true` 进入 model passthrough（会关闭 `GET /v1/models`；部分客户端可能依赖该接口）。自用模式（`REALMS_SELF_MODE_ENABLE=true`）下数据面会进入“宽松的 model passthrough”：不要求模型定义/定价；默认透传 `model`，但若配置了“渠道绑定模型”（alias/upstream_model），则仍会按选中渠道对 `model` 做转发改写。

### 5. 创建数据面 Token（默认模式）

登录后在控制台的 `API 令牌` 页面（`/tokens`）创建数据面令牌（`sk_...`）。令牌默认隐藏，可在列表页查看/复制；撤销后无法查看。升级前创建的旧令牌可能无法显示明文，需要重新生成后才能查看。

自用模式下不提供 Token/账号系统：首次打开 `/login` 设置管理 Key，之后客户端与管理面都直接使用该 Key 作为 `Authorization: Bearer <key>`（或 `x-api-key`）。

### 6. 用 curl 测试（OpenAI 兼容）

```bash
# 默认模式：Authorization 使用数据面 Token（sk_...）
# 自用模式：Authorization 使用管理 Key（在 /login 设置）
...
自用模式（`REALMS_SELF_MODE_ENABLE=true`）下会收敛到最小可用：不提供账号/Token/系统设置/OAuth 等功能域；管理后台仅保留上游渠道（`/admin/channels`）与用量统计（`/admin/usage`），并通过 `/login` 的管理 Key 解锁访问。
...
1) 配置 OpenAI 环境变量：
- 默认模式：先在控制台创建数据面令牌（`sk_...`）
- 自用模式：使用你在 `/login` 设置的管理 Key

Linux/macOS（bash/zsh）：

```bash
export OPENAI_BASE_URL="http://localhost:8080/v1"
export OPENAI_API_KEY="sk_..." # 自用模式下替换为你的管理 Key
```
...
$env:OPENAI_API_KEY = "sk_..." # 自用模式下替换为你的管理 Key

# 持久化到用户环境变量（新终端生效）
[System.Environment]::SetEnvironmentVariable("OPENAI_BASE_URL", "http://localhost:8080/v1", "User")
[System.Environment]::SetEnvironmentVariable("OPENAI_API_KEY", "sk_...", "User") # 自用模式下替换为你的管理 Key
```
...
- 自用模式：`REALMS_SELF_MODE_ENABLE=true`  
  适合个人/小团队自用：禁用订阅/支付/工单等功能域；管理后台无需账号登录（首次打开 `/login` 设置管理 Key，后续用 Key 解锁）；数据面进入 free mode（不校验订阅/余额，仅记录用量）；数据面对 `model` 采用宽松模式（不要求模型定义/定价，默认透传，绑定存在时仍可改写）。
- 默认模式：`REALMS_SELF_MODE_ENABLE=false`  
